...
class MouseReader(file_dispatcher):
    def __init__(self, device, parent):
        self.parent = parent
        self.device = InputDevice(device)
        file_dispatcher.__init__(self, self.device)
        print("Using:")
        print(self.device)
    ...
    def handle_read(self):
        for event in self.recv():
            ...
            if(int(repr(event.type)) == 2):
                print(repr(event.code))
                if(int(repr(event.code)) == 0):
                    # move on X-axis
                    print("move X with", int(repr(event.value)[:-1]))
                    self.parent.steer(int(repr(event.value)[:-1]))
                elif(int(repr(event.code)) == 1):
                    #move on Y-axis
                    print("move Y with", int(repr(event.value)[:-1]))
...
class controller():
    def __init__(self):
        ...
        MouseReader('/dev/input/event2', self)
        loop()

    def steer(self,val):
        if(val < 0):
            self.granTurino.steer("right")
        else:
            self.granTurino.steer("left")
        self.granTurino.steeringAngle(0.8)
